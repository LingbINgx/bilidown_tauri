...
- **视频下载**: 支持通过 BV 号或链接下载 Bilibili 视频。
- **番剧下载**: 支持通过链接下载 Bilibili 番剧/剧集。
- **画质选择**: 自动获取可用画质，默认下载最高画质（如 4K）。
- **二维码登录**: 内置二维码登录功能，支持获取更高画质权限。
- **实时进度**: 显示下载进度、当前速度及预计剩余时间。
- **历史记录**: 自动记录下载历史到本地日志文件。
- **自定义设置**: 支持自定义视频保存路径。
