/// 摘自 https://github.com/SocialSisterYi/bilibili-API-collect/blob/master/docs/misc/sign/wbi.md
/// 感谢 SocialSisterYi 和 SessionHu 大佬的整理
///
use md5;
...
//     // println!("{}", query);
//     let x = get_wbi_keys_main().await.unwrap();
//     println!("{:?}", x.wts);
//     println!("{:?}", x.w_rid);
// }
...
pub async fn get_wbi_keys_main() -> Result<WbiKey, reqwest::Error> {
    let keys = get_wbi_keys().await?;
    let params = vec![];
    let query = encode_wbi(params, keys);

    let wts = query.split('&').find(|x| x.starts_with("wts=")).unwrap();
    let w_rid = query.split('&').find(|x| x.starts_with("w_rid=")).unwrap();

    Ok(WbiKey {
        wts: wts.split('=').last().unwrap().to_string(),
        w_rid: w_rid.split('=').last().unwrap().to_string(),
    })
}
